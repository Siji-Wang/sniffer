...
void Capture_qq::run()
{
  ...
  while(flag_run==true&&(res=sniffer->captureOnce())>=0){
    ...
    if (ih->proto == UDP_SIG) {
        ...
        if (*pByte == QQ_SIGN && (sport == 8000 || dport == 8000)) {
    ...
    else if(ih->proto == TCP_SIG){
      th = (tcp_header *)((unsigned char *)ih + ip_len);      // 获得TCP 协议头
      sport = ntohs(th->src_port);                               // 获得源端口和目的端口
      dport = ntohs(th->dst_port);
      pByte = (unsigned char *)(ih + ip_len + sizeof(tcp_header));
      if (*pByte == QQ_SIGN && (sport == 8000 || dport == 8000)) {
            QQnumber = *(int *)(pByte + QQ_NUM_OFFSET);
            tmp.qq_num = QQnumber;
          }
